    ...
    this.subscriptions.push( this.messageService.changeEmitter
      .subscribe( () => {
        ...
        if ( messages.length <= this.messages.length ||
          ( this.messages.length > 0 && messages.length > 0 &&  messages[0]._id === this.messages[0]._id ) ) {
          this.single = true;
          this.paginate = false;
        } else if ( this.messages.length > 0 && messages.length > 0 &&  messages[0]._id !== this.messages[0]._id ) {
          this.paginate = true;
          this.single = false;
        } else {
          this.paginate = false;
